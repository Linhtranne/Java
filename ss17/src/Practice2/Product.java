...
public class Product {
    ...
    private String inputProductId(Scanner scanner) {
        ...
        do {
            ...
            id = scanner.nextLine();
            if (!Validate.isValidProductId(id)) {
                System.out.println("Mã sản phẩm không hợp lệ hoặc bị trùng!");
            }
        } while (!Validate.isValidProductId(id));
        return id;
    }
    ...
    private String inputProductName(Scanner scanner) {
        ...
        } while (!Validate.isValidProductName(name));
        return name;
    }
    ...
    private float inputImportPrice(Scanner scanner) {
        ...
        do {
            ...
            if (!Validate.isValidImportPrice(price)) {
                System.out.println("Giá nhập phải lớn hơn 0!");
            }
        } while (!Validate.isValidImportPrice(price));
        return price;
    }
    ...
    private float inputExportPrice(Scanner scanner) {
        ...
        do {
            ...
            price = Float.parseFloat(scanner.nextLine());
            if (!Validate.isValidExportPrice(price, importPrice)) {
                System.out.println("Giá xuất phải lớn hơn ít nhất 20% so với giá nhập!");
            }
        } while (!Validate.isValidExportPrice(price, importPrice));
        return price;
    }
    ...
    private int inputQuantity(Scanner scanner) {
        ...
        do {
            ...
            if (!Validate.isValidQuantity(qty)) {
                System.out.println("Số lượng phải lớn hơn 0!");
            }
        } while (!Validate.isValidQuantity(qty));
        return qty;
    }
